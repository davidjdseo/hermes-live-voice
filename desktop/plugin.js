...
export class VoiceCore {
  constructor({ onSubmit = () => {}, onRecord = () => {}, onSpeak = () => {}, onVoice = () => {}, onStatus = () => {}, scheduler = globalThis, rearm = {} } = {}) {
    this.hooks = { onSubmit, onRecord, onSpeak, onVoice, onStatus }; this.scheduler = scheduler; this.minRearmMs = rearm.min ?? 500; this.maxRearmMs = rearm.max ?? 30000; this.noBlockRearmMs = rearm.noBlock ?? 700;
    this.phase = PHASES.OFF; this.sessionId = null; this.armed = false; this.afterReply = false; this.lastSpoken = ''; this.lastQuestionChoices = new Set(); this.parts = []; this.turnId = null; this.completedTurns = new Set(); this.lastAccepted = ''; this.lastRejection = ''; this.rearmTimer = null;
  }
  ...
  cancelRearm() { if (this.rearmTimer !== null) { this.scheduler.clearTimeout(this.rearmTimer); this.rearmTimer = null; } }
  scheduleRearm(ms) { if (this.rearmTimer !== null) return; const delay = Math.max(this.minRearmMs, Math.min(this.maxRearmMs, ms)); this.rearmTimer = this.scheduler.setTimeout(() => { this.rearmTimer = null; if (this.sessionId && [PHASES.IDLE, PHASES.LISTENING, PHASES.SPEAKING].includes(this.phase)) { this.status(PHASES.IDLE); this.hooks.onRecord('start', this.sessionId); } }, delay); }
  ...
  resetTurnState() { this.parts = []; this.turnId = null; this.completedTurns.clear(); this.lastSpoken = ''; this.lastQuestionChoices = new Set(); this.afterReply = false; }
